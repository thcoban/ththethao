    ...
    try:
        with open(file_name, "w", encoding="utf-8") as file:
            ...
            for channel in channels:
                ...
                stream_links = channel["sources"][0]["contents"][0]["streams"][0]["stream_links"]

                #print_colored(f'channel["sources"]: {channel["sources"]}', "yellow")
                #print_colored(f'channel["sources"]["contents"]["streams"]: {stream_links}', "yellow")
                
                for stream_link in stream_links:
                    request_headers = stream_links[0].get("request_headers", None)
                    channel_url = stream_links[0].get("url", "#https://localhost")
